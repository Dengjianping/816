	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn exchange(origin, exchange_symbol: Symbol, amount: u128, target_symbol: Symbol) {
            let exchanger = ensure_signed(origin)?;
                ...
                <token::AccountToken<T>>::contains_key((&exchanger, exchange_symbol)) && 
                <token::AccountToken<T>>::contains_key((&exchanger, target_symbol)),
                Error::<T>::UserHasNoThisToken
            );
            ensure!(
                // T::TokenTrait::get_token(&exchanger, exchange_symbol).balance >= amount,
                <token::AccountToken<T>>::get((&exchanger, target_symbol)).balance >= amount,
			...
			match (exchange_symbol, target_symbol) {
                (Symbol::USD, Symbol::RMB) => {
                    ...
                    <token::AccountToken<T>>::mutate((&exchanger, exchange_symbol), |token| {
                        token.balance = token.balance.saturating_sub(amount);
                    });
                    // issue rmb to exchanger
                    // T::TokenTrait::increase(&exchanger, target_symbol, rmb);
                    <token::AccountToken<T>>::mutate((&exchanger, target_symbol), |token| {
                        token.balance = token.balance.saturating_add(rmb);
                    });
                ...
                (Symbol::RMB, Symbol::USD) => {
                    ...
                    <token::AccountToken<T>>::mutate((&exchanger, target_symbol), |token| {
                        token.balance = token.balance.saturating_add(usd);
                    });
